/* presentes.js — versão revisada e corrigida
   ...
   — BR Code compatível com bancos
   — Modal com scroll
   — Layout do modal revisado (imagem menor e quadrada)
   — Campos alinhados
   — 120 itens
*/
...
function gerarBRCodePix({ chave, nome, cidade, valor, txid, pointOfInit = PIX_POINT_OF_INIT }){
  ...
  const mcc = f('52','0000');
  const curr = f('53','986');
  const val = valor ? f('54', valor.toFixed(2)) : '';
  const c = f('58','BR');
...
function bindControls(){
  ...
  if(cat && !cat.dataset.bound){ cat.addEventListener
